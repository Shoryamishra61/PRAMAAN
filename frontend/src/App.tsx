...
import { FeclResearchPanel } from "./FeclResearchPanel";
...
function ResearchCurve({
  ...
  const polyline = points
    .map((point) => `${point[x] * 100},${100 - point[y] * 100}`)
    .join(" ");
  return (
    <svg
      className="ai-curve"
      ...
      aria-label={label}
    >
      <path d="M0 100H100M0 100V0" className="ai-axis" />
      <polyline points={polyline} className="ai-line" />
    </svg>
  );
}
...
function DecisionEngineView({
  ...
  const [fecl, setFecl] = useState<FeclV2Response | null>(null);
  const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!resolvedCaseId)
          throw new Error(
            "No local BLOCK case is available for the Decision Engine.",
          );
        const [caseResult, researchResult, feclResult] = await Promise.all([
          fetchAiLab(resolvedCaseId),
          fetchAiResearch(),
          fetchFeclV2().catch(() => null),
        ]);
        if (active) {
          setLab(caseResult);
          setResearch(researchResult);
          setFecl(feclResult?.test?.models ? feclResult : null);
        }
      } catch (reason: unknown) {
        if (active)
          setError(
  ...
  const models = artifact
    ? ([
        ...
        ["TF-IDF logistic", "tfidf", artifact.claim_extraction.tfidf.combined],
        [
          "MiniLM + logistic",
          "embedding",
        ...
        ["XGBoost stack", "xgboost", artifact.claim_extraction.xgboost_stack],
        [
    ...
    : [];
  const activeModel =
    models.find((model) => model[1] === selectedModel) ?? models[0];
  ...
  return (
    <>
      ...
      <section className="ai-lab" aria-labelledby="ai-lab-title">
        ...
        {(!lab || !research) && !error && (
          <p role="status" className="product-loading">
            Loading saved local model evidence…
          </p>
        )}
          ...
          <>
            ...
            <section className="ai-research-panel">
              <div className="ai-panel-head">
                ...
                <span className="product-mono">
                  DEV · GENERATED {artifact.created_at.slice(0, 10)}
                </span>
              </div>
              <div className="ai-table-wrap">
                <table className="ai-leaderboard">
                  <thead>
                    <tr>
                      <th>Candidate</th>
                      <th>P</th>
                  ...
                  <tbody>
                    {models.map(([name, id, model]) => (
                      <tr key={id} data-selected={id === "regex"}>
                        <th>{name}</th>
                        <td>{percent(model.metrics.precision)}</td>
                        <td>{percent(model.metrics.recall)}</td>
                        <td>{percent(model.metrics.f1)}</td>
                        <td>
                          {model.latency
                            ? `${model.latency.p95_ms.toFixed(3)} ms`
                            : "n/a"}{" "}
                          ·{" "}
                          {model.model_bytes
                            ? `${(model.model_bytes / 1_000_000).toFixed(2)} MB`
                            : "n/a"}
                        </td>
            ...
            <section className="ai-switcher">
              ...
              {example && activeModel && (
                <div className="ai-debug-grid">
                  <article className="ai-evidence-debug">
                    <span>EXACT SOURCE SENTENCE</span>
                    <blockquote>“{example.text}”</blockquote>
                    <code>{example.example_id}</code>
                  </article>
                  <article className="ai-model-debug">
                    <span>MODEL FINDING · {activeModel[0]}</span>
                    <strong>
                      {(score ?? 0) >= 0.5
                        ? "PROCESSED CLAIM"
                        : "NO CLAIM / ABSTAIN"}
                    ...
                    <p>
                      Model score {(score ?? 0).toFixed(3)} · threshold 0.500
                    </p>
                  </article>
                  <article className="ai-truth-debug">
                    <span>LABELED SEMANTIC TRUTH</span>
                    <strong>
                      {example.label ? "PROCESSED CLAIM" : "NO PROCESSED CLAIM"}
                    </strong>
                    <p>
                      {Number((score ?? 0) >= 0.5) === example.label
                        ? "MATCH"
                        : "ERROR"}{" "}
                      · exact quote remains the audit anchor
                    </p>
                  </article>
                </div>
              )}
                ...
                <section className="ai-research-panel">
                  ...
                  {activeModel[2].precision_recall_curve ? (
                    <ResearchCurve
                      ...
                      points={activeModel[2].precision_recall_curve.points}
                      x="recall"
                      y="precision"
                    />
                  ) : (
                    <p>Rules emit decisions, not a score curve.</p>
                  )}
                </section>
                <section className="ai-research-panel">
                  ...
                  {(
                    ...
                    <>
                      <ResearchCurve
                        ...
                        points={
                          (activeModel[2].calibration ??
                            activeModel[2].raw_calibration)!.bins
                        }
                        x="mean_probability"
                        y="positive_rate"
                      />
                      <p className="ai-caveat">
                        Brier{" "}
                        {(activeModel[2].calibration ??
                          activeModel[2].raw_calibration)!.brier.toFixed(
                          3,
                        )}{" "}
                        · ECE{" "}
                        {(activeModel[2].calibration ??
                          activeModel[2].raw_calibration)!.ece_5.toFixed(3)}
                      </p>
                    </>
                  ) : (
                    <p>No probabilistic output.</p>
                  )}
                </section>
                <section className="ai-research-panel">
                  ...
                  {(
                    ...
                    <>
                      <ResearchCurve
                        ...
                        points={
                          (activeModel[2].risk_coverage ??
                            activeModel[2].raw_risk_coverage)!.points
                        }
                        x="coverage"
                        y="risk"
                      />
                      <p className="ai-caveat">
                        AURC{" "}
                        {(activeModel[2].risk_coverage ??
                          activeModel[2].raw_risk_coverage)!.aurc.toFixed(
                          3,
                        )}{" "}
                        · conformal coverage{" "}
                        {activeModel[2].conformal
                          ? percent(activeModel[2].conformal.empirical_coverage)
                          : "n/a"}
                      </p>
                    </>
                  ) : (
                    <p>Rules do not manufacture uncertainty.</p>
                  )}
                </section>
              </div>
            )}

            <section className="ai-robustness-strip">
              <div>
                <span>OOD CHALLENGE</span>
                <strong>
                  {percent(
              ...
              <div>
                <span>EXTERNAL INFERENCE COST</span>
                <strong>$0.00</strong>
                <small>Local CPU experiment; electricity not monetized</small>
              </div>
              <div>
                <span>GROUNDING INVARIANT</span>
                <strong>Exact and unique</strong>
                <small>Repeated quotes fail closed to REVIEW</small>
              </div>
            </section>
            ...
            <div className="ai-story-grid">
              <section className="ai-story">
                  ...
                  {artifact.claim_extraction.xgboost_stack.tree_shap?.global_mean_absolute
                    ...
                    .map((item) => (
                      <div key={item.feature}>
                        ...
                        <i
                          style={{
                            width: `${Math.min(100, (item.mean_abs_shap / 3.1) * 100)}%`,
                          }}
                        />
                ...
                <p className="ai-caveat">
                  TreeSHAP attributes this fitted score. It does not prove
                  causal reasoning.
                </p>
              </section>
              <section className="ai-story">
                ...
                <div className="ai-metrics">
                  <div>
                    ...
                    <strong>
                      F1{" "}
                      {percent(
                        artifact.contradiction_detection.cross_encoder.metrics
                          .f1,
                      )}
                    </strong>
                    <small>
                  ...
                  <div>
                    <span>Literal rules</span>
                    <strong>
                      F1{" "}
                      {percent(
                        artifact.contradiction_detection.literal_baseline
                          .metrics.f1,
                      )}
                    </strong>
                    <small>20 synthetic frozen pairs</small>
                  </div>
                </div>
                <p className="ai-caveat">
                  Misses remain in amount, reference, and temporal relations.
                  Experimental only.
                </p>
              </section>
            ...
            <section className="ai-research-panel">
              ...
              <p className="ai-digest product-mono">
                ARTIFACT SHA-256 · {research.artifact_sha256}
              </p>
            </section>
            <div className="ai-next">
              <div>
                <h2>
                  AI proposes language. Deterministic code and people keep
                  authority.
                </h2>
                <p>
                  The weaker candidate remains visible for inspection but is not
                  promoted into the gate.
                </p>
              </div>
              <button className="product-primary" onClick={onWorkspace}>
                Inspect the real case <ArrowRight size={17} />
              </button>
            </div>
          </>
        )}
...
export const AiLab = DecisionEngineView;
