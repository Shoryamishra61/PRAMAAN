...
export interface AiResearchResponse {
  ...
  artifact: {
    ...
    orchestration: {
      engine: string;
      authority: string;
      trace: Array<{ node: string; latency_ms: number }>;
    };
...
export interface FeclV2Response {
  generated: true;
  test_sha256: string;
  analysis_sha256: string;
  test: {
    created_at: string;
    boundary: { split: "TEST"; synthetic: true; runtime_changed: false };
    promotion: {
      status: string;
      reason: string;
      selected_runtime: string;
      runtime_changed: false;
      eligible_research_models: string[];
    };
    models: Record<
      string,
      {
        architecture: string;
        metrics: FeclMetricSet;
        calibrated_metrics: FeclMetricSet;
        latency: { p50_ms_per_case: number; p95_ms_per_case: number };
      }
    >;
    predictions: Array<{
      case_id: string;
      pair_id: string;
      counterfactual_case_id: string;
      family: string;
      phenomenon: string;
      ...
      semantic_state_prediction: string;
      scores: Record<string, number>;
      calibrated_scores: Record<string, number>;
    }>;
    ood: {
      count: number;
      learned_only_rejection_rate: number;
      combined_safe_controller_rejection_rate: number;
      by_category: Record<string, { count: number; combined_rejected: number }>;
    };
  };
  analysis: {
    tuning_performed: false;
    counterfactual_pairs: Record<
      string,
      {
        pairs: number;
        both_correct_rate: number;
        decision_changed_rate: number;
      }
    >;
    slices: {
      family: Record<string, Record<string, FeclMetricSet & { count: number }>>;
    };
    calibration_delta: Record<
      string,
      { brier_delta: number; ece_delta: number; f1_delta: number }
    >;
  };
}

export async function fetchFeclV2(): Promise<FeclV2Response> {
  return readJson<FeclV2Response>(
    await apiFetch("/api/v1/ai-research/fecl-v2"),
  );
}
...
export interface QuantRiskResponse {
  ...
  daily_risk_budget_consumed_pct: number;
  review_capacity_utilized_pct: number;
  primary_hypothesis: {
    statement: string;
  ...
  baseline_ladder: Array<{
    ...
    transaction_weighted_cost: number;
    cvar_95: number;
    cvar_99: number;
  }>;
  policy_frontier: Array<{
    preset_name: string;
  ...
  merchant_economics: {
    annual_dispute_volume: number;
    mean_dispute_amount_inr: number;
    baseline_rules_annual_loss_inr: number;
    carve_decision_loss_annual_inr: number;
    gross_merchant_edge_inr: number;
    frictions: {
      analyst_review_cost_inr: number;
      evidence_acquisition_cost_inr: number;
      cloud_compute_cost_inr: number;
      total_friction_inr: number;
    };
    net_merchant_edge_inr: number;
    net_merchant_edge_percent: number;
    return_on_operational_investment: number;
  };
  tail_risk: {
    finding: string;
    models: Array<{
  ...
  disagreement: {
    finding: string;
    p_error_given_agreement: number;
    p_error_given_disagreement: number;
    z_statistic: number;
    p_value: number;
    hypothesis_h3_supported: boolean;
  };
  ...
  calibration: {
    raw_ece: number;
    platt_ece: number;
    temp_scaled_ece: number;
    optimal_temperature: number;
    brier_reduction_pct: number;
  };
  evidence_acquisition_voi: {
    greedy_voi_resolution_rate: number;
    cost_per_resolved_case_inr: number;
    static_checklist_cost_inr: number;
    cost_savings_pct: number;
  };
  stress_tests: {
    scenarios_evaluated: number;
    circuit_breaker_triggered: boolean;
    max_observed_tail_loss: number;
    recommendation: string;
  };
  regimes: {
    monitored_regimes: number;
    edge_half_life_months: number;
    drift_trigger_threshold: number;
  };
  attribution: {
    residual_loss_categories: Array<{
      cause: string;
      pct_contribution: number;
      primary_mitigation: string;
    }>;
  };
  externality_matrix?: Record<string, unknown>;
  learning_curves?: Record<string, unknown>;
  sample_efficiency?: Record<string, unknown>;
  merchant_monte_carlo?: Record<string, unknown>;
  rule_holdout?: Record<string, unknown>;
  training_manifest?: Record<string, unknown>;
}

export async function fetchQuantRiskResearch(): Promise<QuantRiskResponse> {
  return readJson<QuantRiskResponse>(
    await apiFetch("/api/v1/research/quant-risk"),
  );
}
